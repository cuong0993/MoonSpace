...
class _CoolSwiperState extends State<CoolSwiper>
  ...
  List<Widget> get _stackChildren => List.generate(_cards.length, (i) {
  ...
  void _onAnimationTrigger() async {
    ...
    Future.delayed(Constants.backgroundCardsAnimationDuration).then((_) {
      _backgroundCardsAreInFrontNotifier.value = true;
    });
    Future.delayed(Constants.swipeAnimationDuration).then((_) {
  ...
  Widget _buildBackgroundCardsStack() {
    return Stack(
      children: List.generate(_cards.length - 1, (i) => _buildStackChild(i)),
    );
  }
