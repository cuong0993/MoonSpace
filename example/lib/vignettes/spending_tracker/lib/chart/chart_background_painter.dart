...
class ChartBackgroundPainter extends CustomPainter {
  ...
  ChartBackgroundPainter(Chart chart) : _chart = chart, super(repaint: chart);
  ...
  void paint(canvas, size) {
    ...
    for (
      int i = _chart.domainStart.round();
      i <= _chart.domainEnd.round();
      ++i
    ) {
    ...
    canvas.clipRect(
      Rect.fromLTRB(16, size.height, size.width - 16, size.height + 24),
    );
    for (int i = start.round(); i <= end.round(); ++i) {
      ...
      final textPainter = TextPainter(
        text: TextSpan(
          ...
          style: text1.copyWith(
            color: i == _chart.selectedDataPoint
                ? Color(0xFFC3C8D9)
                : Color(0xFFC4C8D9),
            fontWeight: i == _chart.selectedDataPoint
                ? FontWeight.bold
                : FontWeight.w200,
          ),
        ),
