...
class _SpendingGraphState extends State<SpendingGraph>
  ...
  Widget build(context) {
    ...
    final List<String> yAxisLabels = ['\$8k', '\$6k', '\$4k', '\$2k', '0'];
    String? label0Text = '', label1Text = '';
    double label0Y = 0, label1Y = 0;
    if (widget.chart.selectedDataPoint != -1) {
      label0Text = numberToPriceString(
        (widget.chart.dataSets[0].values[widget.chart.selectedDataPoint] * 1000)
            .round(),
      );
      label1Text = numberToPriceString(
        (widget.chart.dataSets[1].values[widget.chart.selectedDataPoint] * 1000)
            .round(),
      );
  ...
  void _handleDrag(DragUpdateDetails details) {
    ...
    if (widget.chart.domainStart + d < 0 ||
        widget.chart.domainEnd + d >= widget.chart.maxDomain)
      return;
    if (d < 0) {
