...
class CityScenery extends StatelessWidget {
  ...
  const CityScenery({Key? key, this.animationValue = 0, required this.city})
    : super(key: key);
  ...
  Widget _buildBackgroundTransition(Animation<double> animation) {
    var gradientStart = ColorTween(
      begin: city.color,
      end: Color(0xFFfde9c8),
    ).animate(CurvedAnimation(curve: Curves.easeOut, parent: animation));
    var gradientEnd = ColorTween(
      begin: city.color,
      end: Color(0xFFfdf8f1),
    ).evaluate(animation);
    var borderRadiusAnimation = Tween<double>(
      begin: Styles.cardBorderRadius,
      end: 0,
  ...
  Widget _buildRoadTransition(Animation<double> animation, Size screenSize) {
    ...
    return FadeTransition(
      opacity: Tween<double>(begin: 0, end: 1).animate(
        CurvedAnimation(
          curve: Interval(.7, 1, curve: Curves.easeIn),
          parent: animation,
        ),
      ),
  ...
  Widget _buildCloudsAnimation(Animation<double> animation) {
    return FadeTransition(opacity: animation, child: _Clouds());
  }

  Widget _buildCityAndTreesTransition(
    ...
    var sizeTransition = Tween(begin: sizeStart, end: sizeEnd).animate(
      CurvedAnimation(
        curve: Interval(.25, 1, curve: Curves.easeIn),
        parent: animation,
      ),
    );
    ...
    var cityPositionTransition =
        Tween(
          begin: Offset(0, -screenSize.height * .112),
          end: Offset.zero,
        ).animate(
          CurvedAnimation(
    ...
    //Trees Animations
    var treesOpacityTransition = Tween<double>(begin: 0, end: 1).animate(
      CurvedAnimation(
  ...
  Widget _buildLeavesAnimation(Animation<double> animation) {
    return FadeTransition(opacity: animation, child: _Leaves());
  }
}
...
class _Leaf extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    var dxPosition = Tween<double>(
      begin: -10,
      end: screenSize.width + 10,
    ).transform(Interval(0, .9, curve: this.curve).transform(animationValue));
    var dyPosition = Tween<double>(
      begin: 0,
      end: pi * 2,
    ).transform(animationValue);
    var rotation = Tween<double>(
      begin: 0,
      end: 360,
...
class _CityImage extends StatelessWidget {
  ...
  const _CityImage({Key? key, required this.size, required this.city})
    : super(key: key);
