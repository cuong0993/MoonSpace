  ...
  void start(AnimationController controller, bool flipY) {
    ...
    for (var i = 0; i < curveCount + 1; i++) {
      ...
      var animSequence = TweenSequence([
        TweenSequenceItem(tween: Tween(begin: 0, end: 0), weight: 10.0),
        TweenSequenceItem(
          tween: Tween(
            begin: 0,
            end: height,
        ...
        TweenSequenceItem(
          tween: Tween(begin: height, end: 0).chain(CurveTween(curve: _ease)),
          weight: 60.0,
        ),
      ]).animate(controller);
...
class LiquidPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    _drawLiquidSim(
      ...
      size,
      0,
      Color(0xffC48D3B).withAlpha(100),
    );
    _drawLiquidSim(
      simulation2,
      canvas,
      size,
      5,
      Color(0xff9D7B32).withAlpha(100),
    );
  }
  ...
  void _drawOffsets(LiquidSimulation simulation, Canvas canvas, Size size) {
    for (var pt in simulation1.endPts) {
      canvas.drawCircle(sizeOffset(pt, size), 4, Paint()..color = Colors.red);
    }
    for (var pt in simulation1.ctrlPts) {
