...
import 'package:example/vignettes/plant_forms/lib/demo.dart';
...
class PlantFormInformation extends StatefulWidget {
  ...
  const PlantFormInformation({Key? key, this.pageSize, this.isHidden = false})
    : super(key: key);
...
class _PlantFormInformationState extends State<PlantFormInformation>
  ...
  List<Widget> _buildCountrySpecificFormElements() {
    var postalTitle = _selectedCountry == "United States"
        ? "Zip Code"
        : "Postal Code";
  ...
  void _updateCountrySubdivision(String country) {
    ...
    if (!values.containsKey(_countrySubdivisionKey) &&
      ...
      values[_countrySubdivisionKey] = CountryData.getSubdivisionList(
        _countrySubdivisionKey,
      )[0];
    }
  }
