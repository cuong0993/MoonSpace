...
import 'package:example/vignettes/_shared/ui/widget_model.dart';
...
class Indie3dModelController extends ChangeNotifier {
  ...
  void _initInstances(VertexMesh torus, VertexMesh star, VertexMesh cube) {
    ...
    _rotations.forEach(
      (vec32.Quaternion quaternion) => quaternion.setAxisAngle(
        vec32.Vector3(
  ...
  void triggerTap(BuildContext context, Offset position, int page) {
    ...
    double cameraZ = 0.0;
    {
      final camNDC = (matProj * matView).transform(
        vec32.Vector4(0.0, 0.0, -2.0, 1.0),
      );
      if (camNDC.w != 0.0) {
        camNDC.x /= camNDC.w;
        camNDC.y /= camNDC.w;
  ...
  void _setTransform() {
    for (int i = 0; i < _meshInstances.length; ++i) {
      ...
      final matModel = vec32.Matrix4.compose(
        _positions[i],
        _rotations[i],
        _scales[i],
      );
