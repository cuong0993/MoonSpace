...
class _TabShellState extends State<TabShell>
  ...
  void initState() {
    ...
    tabcon = TabController(
        vsync: this, length: widget.shellData.length, initialIndex: 0);
  ...
  Widget build(BuildContext context) {
        ...
        ? Column(
            children: [
              if (widget.showTabbar)
                widget.shellData.tabBar(context: context, controller: tabcon),
              Expanded(
                child: tabview,
              ),
            ],
    ...
    return Scaffold(
      appBar: !widget.showAppbar
          ? null
          : AppBar(
              automaticallyImplyLeading: widget.canPop != null,
      ...
      endDrawer: !widget.showDrawer
          ? null
          : widget.shellData
              .navigationDrawer(context: context, controller: tabcon),
      bottomNavigationBar: !widget.showNavigationBar
          ? null
          : widget.shellData
              .bottomNavigationBar(context: context, controller: tabcon),
      body: SafeArea(
        child: childWithRail,
      ),
    );
